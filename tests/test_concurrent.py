...
"""
            ...
            self.lmp.mliappy_load_unified(unified).result()
...
"""
            ...
            self.lmp.command("run 0").result()
            etotal_initial = self.lmp.get_thermo("etotal").result()
            self.assertTrue(np.isfinite(etotal_initial))

            self.lmp.command("run 10").result()
            temp_final = self.lmp.get_thermo("temp").result()
            etotal_final = self.lmp.get_thermo("etotal").result()
