...
"""
This wrapper is required to address the dask actors
"""
...
class DaskLammps:
    def __init__(self, cores=8, working_directory=".", client=None, mode='socket'):
        ...
        if self.mode == 'dask':
            fut = self.client.submit(LammpsLibrary, cores=self.cores, actor=True)
            self.lmp = fut.result()
            ...
            self.lmp = LammpsLibrary(cores=self.cores)
            self.lmp.start_process()

        else:
            raise ValueError("mode should be either dask or local")

    #now we need wrapping for other functions. Lets add

    #route all functions through this higher level
    #class
